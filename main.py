...
import models
from utils import my_dataset, Logger, load_pickle, plot_anomaly_score_distribution, verify_time_shift_robustness, \
    plot_robustness_curve,plot_anomaly_score_distribution_english
from NDCC import NDCC
from HSIC import Classifier
# from HSIC_noisy import Classifier
...
if __name__ == '__main__':
    ...
    parser.add_argument('--dataset', type=str, default='DailySports',
    ...
    output_folder = os.path.join(opt.checkpoint_dir, opt.model, opt.dataset, opt.network, str(opt.seed))
    ...
    if opt.test == False:
        ...
        plot_anomaly_score_distribution_english(ND_scores, ND_labels, ND_pred,
                                        plot_adjusted=False, save_dir=f"./score_view_english/{opt.model}/{opt.dataset}/{opt.seed}/",
                                        error_margin=0.2)
    else:
        # ==================== evaluation ====================
        saved_model_path = os.path.join(output_folder, 'NDCC_state_dict.pth')
        ...
        print(f"总测试时间 (Testing Time): {test_time:.4f} 秒")

        # 计算总参数量
        total_params = sum(p.numel() for p in model.parameters())

        # 如果只想计算可训练的参数量（去掉了被 freeze 的参数）
        # trainable_params = sum(p.numel() for p in model.parameters() if p.requires_grad)

        print(f"模型参数量 (Parameters): {total_params / 1e6:.2f} M")

        dummy_input = torch.randn(128, 125, 45)

        # 如果模型在 GPU 上，输入也要放到 GPU 上
        device = next(model.parameters()).device
        dummy_input = dummy_input.to(device)

        # 计算 MACs 和参数量
        # 如果你的模型除了 input 还需要其他参数，可以写成 inputs=(dummy_input, other_arg)
        macs, params = profile(model, inputs=(dummy_input,))

        # 通常 1 个 MAC (乘加运算) 约等于 2 个 FLOP (一次乘法 + 一次加法)
        # GFLOPs = FLOPs / 10^9
        gflops = (macs * 2) / 1e9

        print(f"计算量 (GFLOPs): {gflops:.3f} GFLOPs")
        ...
        # shifts, res = verify_time_shift_robustness(model, ND_loader)
        # plot_robustness_curve(shifts, res, './robustness/robustness_analysis.svg')
        print('evaluation finished!')
